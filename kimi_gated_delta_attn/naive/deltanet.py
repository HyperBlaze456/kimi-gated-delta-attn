...
def _apply_A(k_t: jax.Array, beta_t: jax.Array, x: jax.Array) -> jax.Array:
    # Applies A_t = (I - beta_t * k_t k_t^T) without forming A_t.
    proj = jnp.einsum("...d,...dk->...k", k_t, x)
    update = jnp.einsum("...d,...k->...dk", k_t, proj)
    return x - beta_t[..., None, None] * update
...
def fused_recurrent_kda(
    ...
    **_unused,
    ...
    beta: (B,H,N) or broadcastable to it.
    """
    ...
    Sf, outT = jax.lax.scan(step, S0, (qT, kT, vT, betaT))
...
def chunk_kda(
    ...
    **_unused,
    ...
    def summary_step(carry, inputs):
        ...
        kt, vt, betat = inputs
        A_accum = _apply_A(kt, betat, A_accum)
        B_accum = _apply_A(kt, betat, B_accum)
        B_accum = B_accum + betat[..., None, None] * jnp.einsum("...d,...v->...dv", kt, vt)
        return (A_accum, B_accum), None

    (A_chunk, B_chunk), _ = jax.lax.scan(summary_step, (A0, B0), (kT, vT, betaT))
    ...
    def scan_chunk(S_init, q_chunk, k_chunk, v_chunk, beta_chunk):
        def step(S, inputs):
            qt, kt, vt, betat = inputs
            S_base = S
            pred = jnp.einsum("bhd,bhdv->bhv", kt, S_base)
        ...
        _, out = jax.lax.scan(step, S_init, (q_chunk, k_chunk, v_chunk, beta_chunk))
        return out

    outT = jax.vmap(scan_chunk, in_axes=(0, 0, 0, 0, 0))(S_beforeT, qT, kT, vT, betaT)
